...
class CosBucket:
    ...
    def upload_simple_file(self, file_body, file_name, dir_name=None):
        """简单上传文件, 无需存储到本地(https://www.qcloud.com/document/product/436/6066)
        ...
        headers = {'Authorization': CosAuth(self.config).sign_more(self.config.bucket, '', 30)}
        r = requests.post(url=self.url, data={'op': 'upload', 'biz_attr': '', 'insertOnly': '0'}, files={
            'filecontent': (file_name, file_body, 'application/octet-stream')}, headers=headers)
        return str(eval(r.content.decode('utf8')).get('data'))
