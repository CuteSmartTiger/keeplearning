      ...
      <v-dialog v-model="dialog" max-width="400px">
        <v-card>
          ...
          <v-card-text>
            ...
            <input id="deploy_files" type="file" name="file"/>
          </v-card-text>
          <v-card-actions>
...
<script>
  ...
  import {listSystem, listUser, createDeploy, uploadDeploy, deleteFileDeploy} from '@/api';
  ...
  export default {
    ...
    methods: {
      ...
      show() {
        document.getElementById('deploy_files').click();
      },
      async upload() {
        let formData = new FormData();
        let files = document.getElementById('deploy_files').files;
      ...
      async deleteFile(deployFile) {
        const res = await deleteFileDeploy({
          ID: deployFile.ID,
          file_name: deployFile.file_name,
        });
        if (res.data.status === 0) {
          if (this.upload_files !== null) {
            let index = this.upload_files.indexOf(deployFile);
