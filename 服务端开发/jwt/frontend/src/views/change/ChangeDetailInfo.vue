<template>
  <v-card color="grey lighten-3" class="mb-5" v-if="item">
    <v-layout row wrap>
      ...
      <v-flex xs6>
        <v-subheader>变更的系统: {{item.system}}</v-subheader>
      </v-flex>
      <v-flex xs6>
        <v-subheader>变更类型: {{item.change_type}}</v-subheader>
      </v-flex>
      <v-flex xs6>
        <v-subheader>变更的版本: {{item.version}}</v-subheader>
      </v-flex>
      <v-flex xs6>
        <v-subheader>计划变更时间: {{item.plan_time | utc2local }}</v-subheader>
      </v-flex>
      ...
      <v-dialog v-model="dialog" max-width="400px">
        <v-card>
          ...
          <v-card-text>
            ...
            <input id="change_files_temp" type="file" name="file"/>
          </v-card-text>
          <v-card-actions>
...
<script>
  import {uploadChange} from '@/api';

  export default {
    name: 'ChangeDetailInfo',
    ...
    methods: {
      show() {
        document.getElementById('change_files_temp').click();
      },
      async upload() {
        let formData = new FormData();
        let files = document.getElementById('change_files_temp').files;
        formData.append('file', files[0]);
        formData.append('change_id', this.item.ID);
